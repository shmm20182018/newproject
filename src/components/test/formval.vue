<template>
  <div>
    <el-form :model="ruleForm" :inline="true" :rules="rules"  ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <component :ruleForm="ruleForm" :rulename="ruleForm[item.rulename]" v-for="(item,index) in items" :is="item.comname" :item="item" :index="index"  :key="item.rulename"></component>
      <el-form-item>
        ...
        this.items.forEach(item => {
          ...
          switch (item.toolid){
            ...
            case 2:
              ...
              break;
            case 3:
              this.rules[item.rulename] = [{type:'daterange',required: item.isRequired, message: '日期范围不能为空', trigger: 'change' }]
              break;
            case 4:
